/*
 * Copyright (c) 2016, 2019, 2022, 2023 Hisao Tamaki
 */
...
public class Graph {
  ...
  /**
   * Prints the adjacency list of the graph using the neighborSet.
   */
  public void printAdjacencyList() {
    ...
    for (int v = 0; v < n; v++) {
      ...
      if (neighborSet[v] != null) {
        for (int neighbor = neighborSet[v].nextSetBit(0);
             neighbor >= 0;
             neighbor = neighborSet[v].nextSetBit(neighbor + 1)) {
  ...
  /**
   * Returns the number of edges of the subgraph of this graph induced
   * by the given vertex set
   * @param vs vertex set
   * @return the number of edges
   */
  ...
  */
  ...
  public void checkConsistency() throws RuntimeException {
    for (int v = 0; v < n; v++) {
      ...
      if (!neighborSet[v].isSubset(all)) {
        throw new RuntimeException("invalid neighbor set of " + v + " " + neighborSet[v] + " n " + n);
      }
      for (int w = 0; w < n; w++) {
...
//      System.out.println("      " + bijection[v] + ":" + g.neighborSet[bijection[v]]);
  ...
  /**
   * print this graph to the standard output in the raw format,
   * where the vertex number starts from 0
   */
  ...
  /**
   * print this graph to the given print streams in the raw format,
   * where the vertex number starts from 0
   * @param ps print stream
   */
  ...
  /**
   * Returns the list of edges as an ArrayList
  ...
  /**
   * Returns the list of edges of the complement graph as an ArrayList
   * @return ArrayList of edges of the complement graph
   */
  public ArrayList<Edge> nonEdgeList() {
    ArrayList<Edge> list = new ArrayList<>();
    for (int v = 0; v < n; v++) {
      XBitSet nb = neighborSet[v];
      for (int w = nb.nextClearBit(v + 1); w < n; w = nb.nextClearBit(w + 1)) {
        list.add(new Edge(v, w, n));
      }
    }
    return list;
  ...
  /**
   * Returns the list of edges as an array
   * @return Edge array
   */
  ...
  /**
   * Returns a set of potential maximal cliques local to the given vertex set
   * as given by a greedy minimal triangulatio of the graph local to the vertex set
   * @param vs vertex set
   * @return the set of potenial maximal cliques
   */
  public Set<XBitSet> localPMCs(XBitSet vs) {
    assert isWellSeparating(vs);
    Set<XBitSet> pmcs = new HashSet<>();
    LocalGraph lg = new LocalGraph(this, vs);
    MMAF mmaf = new MMAF(lg.h);
    mmaf.triangulate();
    Chordal chordal = new Chordal(lg.h);
    for (XBitSet clique: chordal.maximalCliques()) {
      XBitSet pmc = clique.convert(lg.inv);
      assert isPMC(pmc);
      pmcs.add(pmc);
    }
    return pmcs;
